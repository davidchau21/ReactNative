...
const TakeNote = ({route, editingMode, navigation}) => {
    ...
    useEffect(() => {
        fetch(`https://655aff8fab37729791a865ab.mockapi.io/api/v1/takenote/${route.params.id}`, {
            method: 'GET',
    ...
    var deleteTask = (noteId) => {
        ...
        fetch(`https://655aff8fab37729791a865ab.mockapi.io/api/v1/takenote/${userId}`, {
            method: 'GET',
            headers: {'content-type':'application/json'},
        }).then(res => {
            if (res.ok) {
        ...
        <View style={styles.container}>
            <View style={styles.style1}>
                <View style={{flexDirection: 'row', width: '100%', borderWidth: 0, justifyContent: 'space-between', paddingHorizontal: 10}}>
                    <TouchableOpacity style={{backgroundColor: '#48c78e', width: '30%', borderRadius: 10, height: 40, justifyContent: 'center', alignItems: 'center'}}>
            ...
            <View style={styles.style3}>
                <Ionicons name="add-circle-outline" size={80} color="#48c78e" />
            </View>
        </View>
    )
}
